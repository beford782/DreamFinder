...
## Phase 4 — Store logo, PWA icons, favicon (manual / deferred)
...
**PWA icons are not wired up yet.** `manifest.json` currently has **no `icons` array**, and `index.html` references no `icon-192.png` / `icon-512.png`. Generating those and adding them to the manifest is a deferred enhancement (Phase C of the asset-pipeline work), **not** a per-retailer step you perform today. You can collect the retailer's square icon source now for later use, but it is not consumed yet.
